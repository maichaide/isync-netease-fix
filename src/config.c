...
load_config( const char *where )
{
	...
	if (!where) {
		...
		if (config_home)
			nfsnprintf( path, sizeof(path), "%s/.isyncrc", config_home, &path_len );
		else
			nfsnprintf( path, sizeof(path), "%s/.config/.isyncrc", Home, &path_len );
		nfsnprintf( path2, sizeof(path2), "%s/.mbsyncrc", Home, &path_len2 );
		struct stat st;
		...
		if (!sl) {
			nfsnprintf( path, sizeof(path), "./%%%s", &cfile.path_len, where );
			cfile.file = path;
		} else {
